...
companyProfile: {
  ...
  const companyName = companyProfile?.company_name || 'Invoicify';
  ...
  y -= 15;
  lines.push({ size: 11, x: 50, y, text: `Amount: ${formatCurrency(invoice.amount)}` });
  lines.push({ size: 10, x: 50, y: 80, text: 'Generated by Invoicify' });

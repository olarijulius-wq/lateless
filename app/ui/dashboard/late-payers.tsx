...
import {
  fetchDashboardOverdueCustomers,
...
export default async function LatePayers({
  ...
  const latePayers = canView ? await fetchDashboardOverdueCustomers() : [];
            ...
            {isEmpty ? (
              <div className="mt-4 rounded-xl border border-neutral-200 bg-slate-50 p-6 dark:border-zinc-800 dark:bg-black">
                <p className="text-sm text-slate-700 dark:text-slate-200">
                  No overdue customers right now. Customers with unpaid overdue invoices will appear here.
                </p>
              </div>
            ) : (
              <div className="mt-4 space-y-4">
                      ...
                      <div className="flex justify-between gap-4">
                        ...
                        <div className="flex flex-col items-end gap-1">
                          <div className="text-right">
                            <p className="text-xs text-slate-600 dark:text-slate-400">Overdue total</p>
                            <p className="text-sm text-amber-800 dark:text-amber-300">
                              {formatCurrencySuffix(payer.total_overdue_amount)}
                            </p>
                          </div>
                          <div className="text-right">
                            ...
                            <p className="inline-flex items-center rounded-full border border-amber-200 bg-amber-100 px-2 py-0.5 text-xs text-amber-800 dark:border-amber-400/50 dark:bg-amber-500/10 dark:text-amber-300">
                              {payer.overdue_invoices} overdue, {payer.max_days_overdue}d late
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                <div className="hidden overflow-hidden rounded-xl border border-neutral-200 bg-slate-50 md:block dark:border-zinc-800 dark:bg-black">
                  <div className="grid grid-cols-[minmax(0,1.1fr)_minmax(0,1.3fr)_120px_170px] gap-4 border-b border-neutral-200 px-4 py-3 text-xs uppercase tracking-[0.12em] text-slate-500 dark:border-zinc-800">
                    <span>Customer</span>
                    <span>Email</span>
                  ...
                  <div className="max-h-72 divide-y divide-neutral-200 overflow-y-auto dark:divide-zinc-800">
                    {latePayers.map((payer) => (
                      <Link
                        key={payer.customer_id}
                        href={`/dashboard/customers/${payer.customer_id}`}
                        className="grid grid-cols-[minmax(0,1.1fr)_minmax(0,1.3fr)_120px_170px] items-center gap-4 px-4 py-3 text-sm text-slate-700 transition hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-neutral-900"
                      >
                        ...
                        <span className="text-right text-amber-800 dark:text-amber-300">
                          {formatCurrencySuffix(payer.total_overdue_amount)}
                        </span>
